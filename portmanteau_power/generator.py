...
generator.py – Core generation pipeline:
  ...
  5. Diversity selection (MMR-style signature capping)
"""
...
def _mmr_diversify(
    ...
    """Cap how many near-duplicates appear in the final output."""
    out: List[Candidate] = []
...
def generate(
    ...
    # Morpheme bonus list = prefixes + suffixes for scoring
    morpheme_bonus_words = list(set(prefixes + suffixes))
    ...
    def _score_str(
        ...
        total, bd = score_candidate(
            text, ngram_model,
            weights=weights,
            morpheme_bonus_words=morpheme_bonus_words,
        )
        return Candidate(
            text=text,
            score=total,
